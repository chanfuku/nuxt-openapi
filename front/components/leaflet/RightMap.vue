<template>
  ...
  >
    ...
    <l-polygon
      ...
      color="red"
    />
    <template v-for="(line, index) in rightLines">
      <span :key="index">
        <l-marker
...
export default Vue.extend({
  ...
  methods: {
    ...
    addDrawToolbar () {
      ...
      // @ts-ignore
      const drawControl = new L.Control.Draw(this.$leafletHelper.drawToolBarOptionsRight(drawnItems))
      ...
      map.on(L.Draw.Event.CREATED, function (event: any) {
        ...
        self.$store.dispatch('addRightLine', { layerType, latlngs })
      })
    },
    updateLatLng(e: any, index: number, line: { layerType: 'polyline' | 'polygon' | 'rectangle', latlngs: L.LatLng[] }) {
      ...
      this.$store.dispatch('updateRightLine', payload)
    }
  },
  computed: {
    imageUrl () {
    ...
    rightLines () {
      return this.$store.state.rightLines
    }
  },
  watch: {
    leftLine: function(value) {
      // @ts-ignore
      const map = this.$refs?.myMap?.mapObject;
      if (value.layerType === 'rectangle') {
        const bounds = L.latLngBounds(value.latlngs)
